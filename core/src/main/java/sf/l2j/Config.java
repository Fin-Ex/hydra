...
public final class Config {
	...
	public static final String FINEX_FILE = "./config/finex.properties";

	// --------------------------------------------------
	// Finex settings
	// --------------------------------------------------
	public static int[] LINEAGE_REACH_LEVEL;
	public static boolean TALENT_OPENED_FOR_SUBCLASSESS;
	public static int COMBO_PENALTY;
	public static int TALENT_RESET_ID;
	public static int TALENT_RESET_PRICE;
	public static double TALENT_RESET_MODIFIER;
	public static float[] PARTY_EXPSP_BONUS;

	public static boolean OFF_RHYTHM_FEEL_MESSAGE;

	public static boolean RANDOM_QUEST_GENERATOR_ON;
	public static int RANDOM_QUEST_COUNT_PER_GENERATION;
	public static LocalTime RANDOM_QUEST_RESET_TIME;
	...
	/**
	 * itemId1,itemNumber1;itemId2,itemNumber2... to the int[n][2] =
	 ...
	 * Such as olympiad, seven signs festival, four sepulchures, dimensional
	 * rift, weddings, lottery, fishing championship.
	 */
	...
	public static final void saveHexid(int serverId, String hexId, String filename) {
		try {
			...
			_log.warn("Config: Failed to save hex ID to \"" + filename + "\" file.");
			e.printStackTrace();
		}
	}
	 ...
	 * Such as champion monsters, NPC buffer, class master, wyvern, raid bosses
	 * and grand bosses, AI.
	 */
	...
	private static void loadFinex() {
		final ExProperties file = initProperties(FINEX_FILE);
		final String lineagePointsLevelGet = file.getProperty("LINEAGE_REACH_LEVEL", "");
		LINEAGE_REACH_LEVEL = parseIntegerArray(lineagePointsLevelGet, LINEAGE_REACH_LEVEL);
		TALENT_OPENED_FOR_SUBCLASSESS = file.getProperty("TALENT_OPENED_FOR_SUBCLASSESS", true);
		COMBO_PENALTY = file.getProperty("COMBO_PENALTY", 9);
		final String partyExpSpBonus = file.getProperty("PARTY_EXPSP_BONUS", "");
		PARTY_EXPSP_BONUS = parseFloatArray(partyExpSpBonus, PARTY_EXPSP_BONUS);
		TALENT_RESET_ID = file.getProperty("TALENT_RESET_ID", 0);
		TALENT_RESET_PRICE = file.getProperty("TALENT_RESET_PRICE", 0);
		TALENT_RESET_MODIFIER = file.getProperty("TALENT_RESET_MODIFIER", 1.0);

		OFF_RHYTHM_FEEL_MESSAGE = file.getProperty("OFF_RHYTHM_FEEL_MESSAGE", false);

		RANDOM_QUEST_GENERATOR_ON = file.getProperty("RANDOM_QUEST_GENERATOR_ON", true);
		RANDOM_QUEST_COUNT_PER_GENERATION = file.getProperty("RANDOM_QUEST_COUNT_PER_GENERATION", 15);

		// parse the randomq uest timer (regenerate quest time)
		final String[] parser = file.getProperty("RANDOM_QUEST_RESET_TIME", "18:00:00").split(":");
		final int[] hms = new int[parser.length];
		for (int i = 0; i < parser.length; i++) {
			hms[i] = Integer.parseInt(parser[i]);
		}
		switch (hms.length) {
			case 1:
				RANDOM_QUEST_RESET_TIME = LocalTime.of(hms[0], 0, 0);
				break;

			case 2:
				RANDOM_QUEST_RESET_TIME = LocalTime.of(hms[0], hms[1], 0);
				break;

			case 3:
				RANDOM_QUEST_RESET_TIME = LocalTime.of(hms[0], hms[1], hms[2]);
				break;

			default:
				throw new ArrayIndexOutOfBoundsException("Wrong configure of config/finex.properties the RANDOM_QUEST_RESET_TIME - must be setted maximum from 3 values: Hours, Minutes and Seconds.");
		}
	}
	...
	private static int[] parseIntegerArray(String stringArray, int[] baseArray) {
		final String[] splitter = stringArray.split(";");
		baseArray = new int[splitter.length];
		for (int i = 0; i < splitter.length; i++) {
			baseArray[i] = Integer.parseInt(splitter[i]);
		}
		return baseArray;
	}

	private static float[] parseFloatArray(String stringArray, float[] baseArray) {
		final String[] splitter = stringArray.split(";");
		baseArray = new float[splitter.length];
		for (int i = 0; i < splitter.length; i++) {
			baseArray[i] = Float.parseFloat(splitter[i]);
		}
		return baseArray;
	}
}
