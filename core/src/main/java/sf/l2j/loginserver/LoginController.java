...
import sf.l2j.loginserver.crypt.ScrambledKeyPair;
import sf.l2j.loginserver.model.AccountInfo;
...
import sf.l2j.loginserver.network.serverpackets.LoginFail.LoginFailReason;
...
public class LoginController {
	 ...
	 * @throws GeneralSecurityException if a underlying exception was thrown by
	 * the Cipher
	 */
	...
	/**
	 * Update attempts counter. If the maximum amount is reached, it will end
	 * with a client ban.
	 *
	...
	private void recordFailedAttempt(InetAddress addr) {
		...
		if (attempts >= Config.LOGIN_TRY_BEFORE_BAN) {
			...
			_log.warn("IP address: " + addr.getHostAddress() + " has been banned due to too many login attempts.");
		}
	}
	...
	public AccountInfo retrieveAccountInfo(InetAddress addr, String login, String password) {
		try {
			...
			_log.warn("Exception while retrieving account info for '" + login + "'!", e);
			return null;
		}
	}
	...
	public void setAccountAccessLevel(String account, int banLevel) {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			_log.warn("Could not set access_level: " + e.getMessage(), e);
		}
	}
