...
import sf.finex.handlers.talents.Challenger;
...
import sf.l2j.gameserver.model.AbsorbInfo;
...
import sf.l2j.gameserver.model.actor.ai.type.AttackableAI;
import sf.l2j.gameserver.model.actor.ai.type.CreatureAI;
import sf.l2j.gameserver.model.actor.ai.type.SiegeGuardAI;
import sf.l2j.gameserver.model.actor.instance.Servitor;
import sf.l2j.gameserver.model.actor.status.AttackableStatus;
...
import sf.l2j.gameserver.model.item.DropCategory;
import sf.l2j.gameserver.model.item.DropData;
import sf.l2j.gameserver.model.item.instance.type.ItemInstance;
import sf.l2j.gameserver.model.manor.Seed;
...
public class Attackable extends Npc {
	 ...
	 * <ul>
	 * <li>Distribute Exp and SP rewards to Player (including Summon owner) that
	 * hit the L2Attackable and to their Party members</li>
	...
	public boolean doDie(Creature killer) {
		...
		try {
			...
			if (player != null) {
				if (!player.isInParty()) {
					player.getComponent(ComboComponent.class).update(this);
				}
	...
	/**
	 * Distribute Exp and SP rewards to Player (including Summon owner) that hit
	 * the L2Attackable and to their Party members.
	 * <ul>
	 * <li>Get the Player owner of the Servitor (if necessary) and L2Party in
	 * progress</li>
	 * <li>Calculate the Experience and SP rewards in function of the level
	 * difference</li>
	 * <li>Add Exp and SP rewards to Player (including Summon penalty) and to
	 * Party members in the known area of the last attacker</li>
	 * </ul>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T GIVE rewards
	 * to L2PetInstance</B></FONT>
	 *
	...
	protected void calculateRewards(Creature lastAttacker) {
		...
		for (RewardInfo reward : rewards.values()) {
			...
			final Player attacker = reward.getAttacker().getPlayer();
			...
			final float penalty = attacker.hasServitor() ? ((Servitor) attacker.getActiveSummon()).getExpPenalty() : 0;
			...
			if (attackerParty == null) {
				...
				if (!attacker.isDead() && attacker.getKnownType(Attackable.class).contains(this)) {
					...
					if (isOverhit() && _overhitAttacker != null && _overhitAttacker.getPlayer() != null && attacker == _overhitAttacker.getPlayer()) {
					...
					if (Challenger.validate(attacker)) {
						final double modifier = ((Challenger) SkillTable.FrequentTalent.CHALLENGER.getHandler()).invoke();
						exp *= modifier;
						sp *= modifier;
					}
				...
				for (Player partyPlayer : groupMembers) {
					...
					if (reward2 != null) {
						...
						if (partyPlayer.hasPet() && rewards.containsKey(partyPlayer.getActiveSummon())) {
							playersWithPets.put(partyPlayer.getActiveSummon(), rewards.get(partyPlayer.getActiveSummon()));
						}
	 ...
	 * class. The other non-pc instances extend either L2Npc or
	 * L2MonsterInstance.
	 */
	@Override
	...
	public void addDamageHate(Creature attacker, int damage, int aggro) {
		...
		if (aggro == 0) {
			final Player targetPlayer = attacker.getPlayer();
	...
	/**
	 * Reduce hate for the target. If the target is null, decrease the hate for
	 * the whole aggrolist.
	 *
	...
	/**
	 * @return the list of hated Creature. It also make checks, setting hate to
	 ...
	 * @return the hate level of the L2Attackable against this Creature
	 ...
	 * add it Quest drops</li>
	 * <li>For each possible drops (base + quests), calculate which one must be
	 * dropped (random)</li>
	 * <li>Get each Item quantity dropped (random)</li>
	 * <li>Create this or these ItemInstance corresponding to each Item
	 * Identifier dropped</li>
	 * <li>If the autoLoot mode is actif and if the Creature that has killed the
	 * L2Attackable is a Player, give this or these Item(s) to the Player that
	 * has killed the L2Attackable</li>
	 * <li>If the autoLoot mode isn't actif or if the Creature that has killed
	 * the L2Attackable is not a Player, add this or these Item(s) in the world
	 * as a visible object at the position where mob was last</li>
	 * </ul>
	 *
	...
	public void doItemDrop(NpcTemplate npcTemplate, Creature mainDamageDealer) {
		...
		Player player = mainDamageDealer.getPlayer();
		if (player == null) {
			return;
		}
	 ...
	 *
	 * @param attacker The Creature who hit on the L2Attackable using the
	...
	/**
	 * @return the Creature who hit on the L2Attackable using an over-hit
	 * enabled skill.
	 */
	...
	/**
	 * Calculate the Experience and SP to distribute to attacker (Player,
	 * Servitor or L2Party) of the L2Attackable.
	 *
	 * @param diff The difference of level between attacker (Player, Servitor or
	 * L2Party) and the L2Attackable
	 * @param damage The damages given by the attacker (Player, Servitor or
	 * L2Party)
