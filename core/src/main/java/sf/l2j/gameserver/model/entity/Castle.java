...
import sf.l2j.gameserver.model.zone.type.L2CastleTeleportZone;
import sf.l2j.gameserver.model.zone.type.L2CastleZone;
import sf.l2j.gameserver.model.zone.type.L2SiegeZone;
...
public class Castle {
	 ...
	 * @param save : true means we store it on database. Basically setted to
	 * false on server startup.
	 */
	public void setLeftCertificates(int leftCertificates, boolean save) {
		...
		if (save) {
			try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement ps = con.prepareStatement("UPDATE castle SET certificates=? WHERE id=?")) {
				...
				_log.warn("setLeftCertificates: " + e.getMessage(), e);
			}
		}
	}
	 ...
	 *
	 * @param activeChar Sends informative messages to that character (success
	 * or fail).
	...
	public void upgradeDoor(int doorId, int hp, boolean db) {
		...
		if (db) {
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				...
				_log.warn("Exception: saveDoorUpgrade(int doorId, int hp): " + e.getMessage(), e);
			}
		}
	}
	...
	/**
	 * That method is used to spawn NPCs, being neutral guards or player-based
	 * mercenaries.
	 * <ul>
	 * <li>If castle got an owner, it spawns mercenaries following tickets.
	 * Otherwise it uses SpawnManager territory.</li>
	 * <li>It feeds the nearest Control Tower with the spawn. If tower is
	 * broken, associated spawns are removed.</li>
	 * </ul>
	 */
		...
		if (_ownerId > 0) {
			for (ItemInstance item : _droppedTickets) {
				...
				try {
					...
					_log.warn("Could not spawn Npc " + ticket.getNpcId());
				}
	 ...
	 * <ul>
	 * <li>The former clan failed to defend the castle : 1000 points for new
	 * owner, -1000 for former clan.</li>
	 * <li>The former clan successfully defended the castle, ending in a draw :
	 * 500 points for former clan.</li>
	...
	/**
	 * @return the initial castle owner. Used to clean crowns && others castles
	 * related functions.
	 */
