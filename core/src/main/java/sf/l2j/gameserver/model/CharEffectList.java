...
import sf.l2j.gameserver.network.serverpackets.AbnormalStatusUpdate;
import sf.l2j.gameserver.network.serverpackets.ExOlympiadSpelledInfo;
import sf.l2j.gameserver.network.serverpackets.PartySpelled;
...
public class CharEffectList {
	...
	private boolean doesStack(L2Skill checkSkill) {
		...
		final List<EffectTemplate> effectTemplates = checkSkill.getEffectTemplates();
		if (effectTemplates == null || effectTemplates.isEmpty()) {
			return false;
		}

		final String stackType = effectTemplates.get(0).stackType;
	...
	/**
	 * Return the number of buffs in this CharEffectList not counting
	 * Songs/Dances
	 *
	 * @return
	 */
	...
	public void stopEffectsOnDamage(boolean awake) {
		if (_hasBuffsRemovedOnDamage) {
			if (_buffs != null && !_buffs.isEmpty()) {
				for (L2Effect e : _buffs) {
					if (e != null && e.getSkill().isRemovedOnDamage() && (awake || e.getSkill().getSkillType() != ESkillType.SLEEP)) {
						e.exit(true);
					}
				}
			}
	...
	protected void removeEffectFromQueue(L2Effect effect) {
		...
		if ("none".equals(effect.getStackType())) {
			...
			if (index >= 0) {
				...
				if (index == 0) {
					...
					if (!stackQueue.isEmpty()) {
						...
						if (newStackedEffect != null) {
							...
							if (newStackedEffect.setInUse(true)) // Add its list of Funcs to the Calculator set of the Creature
							{
				...
				} else // Update the Stack Group table _stackedEffects of the Creature
				{
	...
	protected void addEffectFromQueue(L2Effect newEffect) {
		...
		if (newSkill.isDebuff()) {
			...
			for (L2Effect buff : _buffs) {
				if (buff.getId() == newEffect.getId()
						...
						&& buff.getStackOrder() == newEffect.getStackOrder()
						&& buff.getStackType().equalsIgnoreCase(newEffect.getStackType())) {
					buff.exit(); // exit this
				}
			}
		...
		if (effectToRemove != effectToAdd) {
			...
			if (effectToAdd != null) {
				// Set this L2Effect to In Use
				if (effectToAdd.setInUse(true)) // Add all Func objects corresponding to this stacked effect to the Calculator set of the Creature
				{
	...
	protected void updateEffectIcons() {
		...
		if (ps != null) {
			if (_owner instanceof Summon) {
				final Player summonOwner = ((Summon) _owner).getPlayer();
				if (summonOwner != null) {
	...
	/**
	 * Clear and null all queues and lists Use only during delete character from
	 * the world.
	 */
	public void clear() {
		_addQueue = null;
