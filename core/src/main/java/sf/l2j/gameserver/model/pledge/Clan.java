package sf.l2j.gameserver.model.pledge;
...
import sf.l2j.gameserver.model.itemcontainer.ClanWarehouse;
...
import sf.l2j.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
import sf.l2j.gameserver.network.serverpackets.PledgeShowInfoUpdate;
import sf.l2j.gameserver.network.serverpackets.PledgeShowMemberListAll;
import sf.l2j.gameserver.network.serverpackets.PledgeShowMemberListDeleteAll;
import sf.l2j.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import sf.l2j.gameserver.network.serverpackets.PledgeSkillList;
import sf.l2j.gameserver.network.serverpackets.PledgeSkillListAdd;
...
public class Clan {
	...
	private static final Logger _log = LoggerFactory.getLogger(Clan.class.getName());
	...
	public Clan(int clanId, int leaderId) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			_log.warn("Error while restoring clan " + e.getMessage());
		}
	 ...
	 * @return the maximum number of members allowed for a given
	 * {@code pledgeType}.
	 */
	...
	private void storeNotice(String notice, boolean enabled) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			_log.warn("L2Clan : could not store clan notice: " + e.getMessage(), e);
		}
	...
	public void setIntroduction(String intro, boolean saveOnDb) {
		if (saveOnDb) {
			...
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				...
				_log.warn("L2Clan : could not store clan introduction: " + e.getMessage(), e);
			}
		}
	...
	/**
	 * Add a new skill to the list, send a packet to all online clan members,
	 * update their stats and store it in db
	 *
	...
	/**
	 * Change the clan crest. If crest id is 0, crest is removed. New crest id
	 * is saved to database.
	 *
	 * @param crestId if 0, crest is removed, else new crest id is set and saved
	 * to database
	 */
	...
	/**
	 * Change the ally crest. If crest id is 0, crest is removed. New crest id
	 * is saved to database.
	 *
	 * @param crestId if 0, crest is removed, else new crest id is set and saved
	 * to database
	...
	/**
	 * Change the large crest. If crest id is 0, crest is removed. New crest id
	 * is saved to database.
	 *
	 * @param crestId if 0, crest is removed, else new crest id is set and saved
	 * to database
