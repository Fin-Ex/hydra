...
/**
 * This class contains an enum of each possible event, that can happen on an AI
 * character.
 */
public enum CtrlEvent {
	/**
	 * Something has changed, usually a previous step has being completed or
	 * maybe was completed, the AI must thing on next action.
	 */
	EVT_THINK,
	/**
	...
	EVT_ATTACKED,
	/**
	 * Increase/decrease aggression towards a target, or reduce global
	 * aggression if target is null.
	 */
	EVT_AGGRESSION,
	/**
	 ...
	 * to physically or magically hit an enemy, or an action that discarded
	 * attack attempt has finished.
	 */
	EVT_READY_TO_ACT,
	/**
	 * The actor arrived to assigned location, or it's a time to modify movement
	 * destination (follow, interact, random move and others intentions).
	 */
	EVT_ARRIVED,
	/**
	 ...
	 * For example, the actor was put into a stun, so it's current attack or
	 * movement has to be canceled. But after the stun state expired, the actor
	 * may try to attack again. Another usage for CANCEL is a user's attempt to
