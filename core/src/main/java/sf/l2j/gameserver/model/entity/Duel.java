...
import sf.l2j.gameserver.model.actor.events.OnDuelEnd;
...
import sf.l2j.gameserver.network.serverpackets.ExDuelEnd;
import sf.l2j.gameserver.network.serverpackets.ExDuelReady;
import sf.l2j.gameserver.network.serverpackets.ExDuelStart;
...
public class Duel {
	...
	/**
	 * This class hold important player informations, which will be restored on
	 * duel end.
	 */
	 ...
	 * <ul>
	 * <li>For 1vs1, the timer begins to 5 (messages then start duel
	 * process).</li>
	 * <li>For party duel, the timer begins to 35 (2sec break, teleport parties,
	 * 3sec break, messages then start duel process).</li>
	 * </ul>
	...
	/**
	 * This task listens the different ways to disturb the duel. Two cases are
	 * possible :
	 * <ul>
	 * <li>DuelResult is under CONTINUE state, nothing happens. The task will
	 * continue to run every second.</li>
	 * <li>DuelResult is anything except CONTINUE, then the duel ends.
	 * Animations are played on any duel end cases, except CANCELED.</li>
	 * </ul>
	 */
	...
	/**
	 * Starts the duel.<br>
	 * Save players conditions, cancel active trade, set the team color and all
	 * duel start packets.<br>
	 * Handle the duel task, which checks if the duel ends in one way or
	 * another.
	 */
	protected void startDuel() {
		if (_partyDuel) {
			for (Player partyPlayer : _playerA.getParty().getMembers()) {
				...
				final Summon summon = partyPlayer.getActiveSummon();
				if (summon != null) {
			...
			Summon summon = _playerA.getActiveSummon();
			if (summon != null) {
				summon.updateAbnormalEffect();
			}
			...
			summon = _playerB.getActiveSummon();
			if (summon != null) {
				summon.updateAbnormalEffect();
			}
		}
	...
	/**
	 * This method ends a duel, sending messages to each team, end duel packet,
	 * cleaning player conditions and then removing duel from manager.
	 *
	...
	/**
	 * This method checks all possible scenari which can disturb a duel, and
	...
	/**
	 * This method is called whenever a player was defeated in a duel. It
	 * updates DuelState of players.
	 *
	...
	/**
	 * This method is called when a player join/leave a party during a Duel, and
	 * enforce Duel cancellation.
	 */
