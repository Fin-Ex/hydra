...
import sf.l2j.gameserver.model.item.instance.FlagPolicy;
import sf.l2j.gameserver.model.item.instance.InventoryFlags;
...
import sf.l2j.gameserver.model.itemcontainer.listeners.OnEquipListener;
import sf.l2j.gameserver.model.itemcontainer.listeners.StatsListener;
...
public abstract class Inventory extends ItemContainer {
	...
	@Getter
	private final InventoryFlags flags = new InventoryFlags();
	...
	public ItemInstance unEquipItemInBodySlot(int slot) {
		...
		if (!flags.checkAccess(slot, FlagPolicy.Unequip)) {
			return null;
		}
	 ...
	 * <B>If you dont need return value use
	 * {@link Inventory#equipItem(ItemInstance)} instead</B>
	 *
	...
	public void equipItem(ItemInstance item) {
		...
		if (!flags.checkAccess(targetSlot, FlagPolicy.Equip)) {
			return;
		}
		...
		if (getOwner().isPlayer()) {
			...
			if (player.isInStoreMode() || (!player.isHero() && item.isHeroItem())) {
				return;
			}
	...
	/**
	 * Equips pet item in slot of paperdoll. Concerning pets, armors go to chest
	 * location, and weapon to R-hand.
	 *
	 * @param item : ItemInstance designating the item and slot used.
	 */
