...
import sf.l2j.gameserver.model.MinionData;
...
public class NpcTemplate extends CreatureTemplate {
	...
	protected static final Logger _log = LoggerFactory.getLogger(NpcTemplate.class.getName());
	...
	public NpcTemplate(StatsSet set) {
		...
		isFlying = set.getBool("isFlying", false);
	}
	...
	/**
	 * @return the list of all possible UNCATEGORIZED drops of this
	 * L2NpcTemplate.
	 */
	...
	/**
	 * @return the list of all possible item drops of this L2NpcTemplate. (ie
	 * full drops and part drops, mats, miscellaneous & UNCATEGORIZED)
	 */
	...
	/**
	 * Add a drop to a given category. If the category does not exist, create
	 * it.
	 *
	 * @param drop
	 * @param categoryType
	...
	/**
	 * @return the list of all Minions that must be spawn with the L2Npc using
	 * this L2NpcTemplate.
	 */
	...
	public void addSkills(List<L2Skill> skills) {
		for (L2Skill skill : skills) {
			...
			// _log.warn(skill.getName() + " skill wasn't added due to specific logic."); TODO
		}
	}
	...
	public void addQuestEvent(EventType type, Quest quest) {
		...
		if (list == null) {
			...
			if (type.isMultipleRegistrationAllowed() || list.isEmpty()) {
				...
				_log.warn("Quest event not allow multiple quest registrations. Skipped addition of EventType \"" + type + "\" for NPC \"" + getName() + "\" and quest \"" + quest.getName() + "\".");
			}
		}
	}
}
