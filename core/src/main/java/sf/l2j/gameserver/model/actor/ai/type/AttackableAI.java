...
import sf.l2j.gameserver.model.actor.instance.FriendlyMonster;
import sf.l2j.gameserver.model.actor.instance.GrandBoss;
import sf.l2j.gameserver.model.actor.instance.Guard;
import sf.l2j.gameserver.model.actor.instance.Monster;
import sf.l2j.gameserver.model.actor.instance.RaidBoss;
import sf.l2j.gameserver.model.actor.instance.RiftInvader;
import sf.l2j.gameserver.model.actor.template.NpcTemplate.AIType;
import sf.l2j.gameserver.model.actor.template.NpcTemplate.SkillType;
...
public class AttackableAI extends CreatureAI implements Runnable {
	...
	/**
	 * The flag used to indicate that a thinking action is in progress ; prevent
	 * recursive thinking
	 */
	 ...
	 * <li>The target isn't dead, isn't invulnerable, isn't in silent moving
	 * mode AND too far (>100)</li>
	...
	/**
	 * Set the Intention of this CreatureAI and create an AI Task executed every
	 * 1s (call onEvtThink method) for this L2Attackable.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : If actor _knowPlayer isn't
	 * EMPTY, IDLE will be change in ACTIVE</B></FONT><BR>
	 * <BR>
	 *
	 ...
	 * <li>If the actor is Aggressive and can attack, add all autoAttackable
	 * Creature in its Aggro Range to its _aggroList, chose a target and order
	 * to attack it</li>
	 * <li>If the actor is a L2GuardInstance that can't attack, order to it to
	 * return to its home location</li>
	 * <li>If the actor is a L2MonsterInstance that can't attack, order to it to
	 * random walk (1/100)</li>
	 * </ul>
	 */
	 ...
	 * <li>If target is dead or timeout is expired, stop this attack and set the
	 * Intention to ACTIVE.</li>
	 * <li>Call all WorldObject of its Faction inside the Faction Range.</li>
	 * <li>Choose a target and order to attack it with magic skill or physical
	 * attack.</li>
	 * </ul>
	 */
	protected void thinkAttack() {
		...
		/**
		 * CAST CHECK<br>
		 * The mob succeeds a skill check ; make all possible checks to define
		 * the skill to launch. If nothing is found, go in MELEE CHECK.<br>
		 * It will check skills arrays in that order :
		 * <ul>
		 * <li>suicide skill at 15% max HPs</li>
		 * <li>buff skill if such effect isn't existing</li>
		 * <li>heal skill if self or ally is under 75% HPs (priority to others
		 * healers and mages)</li>
		...
		if (willCastASpell()) {
			...
			if (!defaultList.isEmpty()) {
				...
				if (npc.isMinion()) {
					...
					if (leader != null && !leader.isDead() && (leader.getCurrentHp() / leader.getMaxHp() < 0.75)) {
						for (L2Skill sk : defaultList) {
							if (sk.getTargetType() == ESkillTargetType.TARGET_SELF) {
								continue;
							}
							...
							if (!MathUtil.checkIfInRange(overallRange, npc, leader, false) && sk.getTargetType() != ESkillTargetType.TARGET_PARTY && !npc.isMovementDisabled()) {
				...
				for (L2Skill sk : defaultList) {
					...
					if (sk.getTargetType() == ESkillTargetType.TARGET_ONE) {
						...
						if (sk.getTargetType() == ESkillTargetType.TARGET_PARTY) {
							clientStopMoving(null);
							npc.doCast(sk, false);
							return;
						}
		...
		/**
		 * MELEE CHECK<br>
		 * The mob failed a skill check ; make him flee if AI authorizes it,
		 * else melee attack.
		 */
	...
	protected boolean cast(L2Skill sk, double distance, int range) {
		...
		switch (sk.getSkillType()) {
			...
			case BALANCE_LIFE: {
				// Minion case.
				if (caster.isMinion() && sk.getTargetType() != ESkillTargetType.TARGET_SELF) {
					...
					if (leader != null && !leader.isDead() && Rnd.get(100) > (leader.getCurrentHp() / leader.getMaxHp() * 100)) {
						...
						if (!MathUtil.checkIfInRange(overallRange, caster, leader, false) && sk.getTargetType() != ESkillTargetType.TARGET_PARTY && !caster.isMovementDisabled()) {
							moveToPawn(leader, overallRange);
						}
			...
			case BLEED: {
				if (GeoEngine.getInstance().canSeeTarget(caster, attackTarget) && !canAOE(sk) && !attackTarget.isDead() && distance <= range) {
					...
					if ((sk.getTargetType() == ESkillTargetType.TARGET_AREA || sk.getTargetType() == ESkillTargetType.TARGET_BEHIND_AREA || sk.getTargetType() == ESkillTargetType.TARGET_FRONT_AREA) && GeoEngine.getInstance().canSeeTarget(caster, attackTarget) && !attackTarget.isDead() && distance <= range) {
						clientStopMoving(null);
						caster.doCast(sk, false);
						return true;
					}
				} else if (sk.getTargetType() == ESkillTargetType.TARGET_ONE) {
					Creature target = targetReconsider(sk.getCastRange(), true);
					if (target != null) {
						clientStopMoving(null);
						caster.doCast(sk, false);
			...
			case PARALYZE: {
				if (GeoEngine.getInstance().canSeeTarget(caster, attackTarget) && !canAOE(sk) && distance <= range) {
					...
					if (sk.getTargetType() == ESkillTargetType.TARGET_AURA || sk.getTargetType() == ESkillTargetType.TARGET_BEHIND_AURA || sk.getTargetType() == ESkillTargetType.TARGET_FRONT_AURA) {
						...
						caster.doCast(sk, false);
						return true;
					} else if ((sk.getTargetType() == ESkillTargetType.TARGET_AREA || sk.getTargetType() == ESkillTargetType.TARGET_BEHIND_AREA || sk.getTargetType() == ESkillTargetType.TARGET_FRONT_AREA) && GeoEngine.getInstance().canSeeTarget(caster, attackTarget) && !attackTarget.isDead() && distance <= range) {
						clientStopMoving(null);
						caster.doCast(sk, false);
						return true;
					}
			...
			case NEGATE: {
				...
				if (sk.getTargetType() == ESkillTargetType.TARGET_ONE) {
					...
					if ((sk.getTargetType() == ESkillTargetType.TARGET_AURA || sk.getTargetType() == ESkillTargetType.TARGET_BEHIND_AURA || sk.getTargetType() == ESkillTargetType.TARGET_FRONT_AURA) && GeoEngine.getInstance().canSeeTarget(caster, attackTarget)) {
						clientStopMoving(null);
						caster.doCast(sk, false);
						return true;
					} else if ((sk.getTargetType() == ESkillTargetType.TARGET_AREA || sk.getTargetType() == ESkillTargetType.TARGET_BEHIND_AREA || sk.getTargetType() == ESkillTargetType.TARGET_FRONT_AREA) && GeoEngine.getInstance().canSeeTarget(caster, attackTarget) && !attackTarget.isDead() && distance <= range) {
	 ...
	 * @return {@code true} if the skill is available for casting {@code false}
	 * otherwise.
	 */
	...
	/**
	 * Method used when the actor can't attack his current target (immobilize
	 * state, for exemple).
	 * <ul>
	 * <li>If the actor got an hate list, pickup a new target from it.</li>
	 * <li>If the actor didn't find a target on his hate list, check if he is
	 * aggro type and pickup a new target using his knownlist.</li>
	 * </ul>
	 *
	 * @param range The range to check (skill range for skill ; physical range
	 * for melee).
	 * @param rangeCheck That boolean is used to see if a check based on the
	 * distance must be made (skill check).
	 ...
	 *
	 * @param oldTarget The previous target, reused if no available target is
	 * found.
	 ...
	 * to 0, Add the attacker to the actor _aggroList</li>
	 * <li>Set the Creature movement type to run and send Server->Client packet
	 * ChangeMoveType to all others Player</li>
	 ...
	 * present</li>
	 * <li>Set the actor Intention to ATTACK (if actor is L2GuardInstance check
	 * if it isn't too far from its home location)</li>
	 * </ul>
	 *
