...
import sf.l2j.gameserver.network.serverpackets.AutoAttackStart;
import sf.l2j.gameserver.network.serverpackets.AutoAttackStop;
import sf.l2j.gameserver.network.serverpackets.Die;
import sf.l2j.gameserver.network.serverpackets.MoveToLocation;
...
abstract class AbstractAI {
	 ...
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method is USED by AI
	 * classes</B></FONT><BR>
	 * <BR>
	 * <B><U> Overridden in </U> : </B><BR>
	 * <B>L2AttackableAI</B> : Create an AI Task executed every 1s (if
	 * necessary)<BR>
	 * <B>L2PlayerAI</B> : Stores the current AI intention parameters to later
	 * restore it if necessary<BR>
	 * <BR>
	 *
	...
	/**
	 * Launch the CreatureAI onIntention method corresponding to the new
	 * Intention.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Stop the FOLLOW mode if
	 * necessary</B></FONT><BR>
	 * <BR>
	 *
	 * @param intention The new Intention to set to the AI
	 ...
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : The current general intention
	 * won't be change (ex : If the character attack and is stunned, he will
	 * attack again after the stunned periode)</B></FONT><BR>
	 * <BR>
	 *
	...
	/**
	 * Cancel action client side by sending Server->Client packet ActionFailed
	 * to the Player actor.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	 * <BR>
	 */
	 ...
	 * Server->Client packet MoveToPawn <I>(broadcast)</I>.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	 * <BR>
	...
	/**
	 * Move the actor to Location (x,y,z) server side AND client side by sending
	 * Server->Client packet CharMoveToLocation <I>(broadcast)</I>.<br>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT>
	 *
	 * @param x
	 ...
	 * Server->Client packet StopMove/StopRotation <I>(broadcast)</I>.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	 * <BR>
	...
	public void setAutoAttacking(boolean isAutoAttacking) {
		if (_actor instanceof Summon) {
			...
			if (summon.getPlayer() != null) {
				summon.getPlayer().getAI().setAutoAttacking(isAutoAttacking);
			}
			return;
		}
	...
	/**
	 * Start the actor Auto Attack client side by sending Server->Client packet
	 * AutoAttackStart <I>(broadcast)</I>.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	 * <BR>
	...
	public void clientStartAutoAttack() {
		if (_actor instanceof Summon) {
			_actor.getPlayer().getAI().clientStartAutoAttack();
			return;
		}

		if (!isAutoAttacking()) {
			if (_actor instanceof Player && ((Player) _actor).getActiveSummon() != null) {
				((Player) _actor).getActiveSummon().broadcastPacket(new AutoAttackStart(((Player) _actor).getActiveSummon().getObjectId()));
			}
	...
	/**
	 * Stop the actor auto-attack client side by sending Server->Client packet
	 * AutoAttackStop <I>(broadcast)</I>.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT>
	 */
	public void clientStopAutoAttack() {
		if (_actor instanceof Summon) {
			_actor.getPlayer().getAI().clientStopAutoAttack();
			return;
		}
	...
	/**
	 * Kill the actor client side by sending Server->Client packet
	 * AutoAttackStop, StopMove/StopRotation, Die <I>(broadcast)</I>.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	 * <BR>
	...
	/**
	 * Update the state of this actor client side by sending Server->Client
	 * packet MoveToPawn/MoveToLocation and AutoAttackStart to the Player
	 * player.<BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	...
	/**
	 * Create and Launch an AI Follow Task to execute every 0.5s, following at
	 * specified range.
	 *
	 * @param target The Creature to follow
	 * @param range
