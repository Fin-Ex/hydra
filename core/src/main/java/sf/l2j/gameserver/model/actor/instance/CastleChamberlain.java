...
import sf.l2j.gameserver.network.serverpackets.ExShowCropInfo;
import sf.l2j.gameserver.network.serverpackets.ExShowCropSetting;
import sf.l2j.gameserver.network.serverpackets.ExShowManorDefaultInfo;
import sf.l2j.gameserver.network.serverpackets.ExShowSeedInfo;
import sf.l2j.gameserver.network.serverpackets.ExShowSeedSetting;
import sf.l2j.gameserver.network.serverpackets.NpcHtmlMessage;
import sf.l2j.gameserver.network.serverpackets.SiegeInfo;
...
public class CastleChamberlain extends Merchant {
	...
	/**
	 * Retrieve the price of the door, following its type, required level of
	 * upgrade and current Seven Signs state.
	 *
	 * @param type : The type of doors (1: normal gates, 2: metallic gates, 3:
	 * walls).
	 * @param level : The required level of upgrade (x2, x3 or x5 HPs).
	 * @return The price modified by Seal of Strife state (-20% if Dawn is
	 * winning, x3 if Dusk is winning).
	 */
