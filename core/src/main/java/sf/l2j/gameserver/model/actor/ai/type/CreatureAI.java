...
import sf.l2j.gameserver.model.actor.ai.Desire;
...
public class CreatureAI extends AbstractAI {
	 ...
	 * packet AutoAttackStop (broadcast)</li>
	 * <li>Stop the actor movement server side AND client side by sending
	 * Server->Client packet StopMove/StopRotation (broadcast)</li>
	 * <li>Stand up the actor server side AND client side by sending
	 * Server->Client packet ChangeWaitType (broadcast)</li>
	 * </ul>
	 */
	@Override
	 ...
	 * new Attack and Launch Think Event.
	 * <ul>
	 * <li>Stop the actor auto-attack client side by sending Server->Client
	 * packet AutoAttackStop (broadcast)</li>
	 * <li>Set the Intention of this AI to ATTACK</li>
	 * <li>Set or change the AI attack target</li>
	 * <li>Start the actor Auto Attack client side by sending Server->Client
	 * packet AutoAttackStart (broadcast)</li>
	 * <li>Launch the Think Event</li>
	 * </ul>
	 */
	@Override
	 ...
	 * cast and Launch Think Event.
	 * <ul>
	 * <li>Set the AI cast target</li>
	 * <li>Stop the actor auto-attack client side by sending Server->Client
	 * packet AutoAttackStop (broadcast)</li>
	 * <li>Cancel action client side by sending Server->Client packet
	 * ActionFailed to the Player actor</li>
	 ...
	 * Server->Client packet AutoAttackStop (broadcast)</li>
	 * <li>Set the Intention of this AI to MOVE_TO</li>
	 * <li>Move the actor to Location (x,y,z) server side AND client side by
	 * sending Server->Client packet MoveToLocation (broadcast)</li>
	 * </ul>
	 */
	@Override
	 ...
	 * <li>Move the actor to Pawn server side AND client side by sending
	 * Server->Client packet MoveToPawn (broadcast)</li>
	 * </ul>
	 */
	@Override
	 ...
	 * <li>Stop the actor movement server side AND client side by sending
	 * Server->Client packet StopMove/StopRotation (broadcast)</li>
	 * <li>Break an attack and send Server->Client ActionFailed packet and a
	 * System Message to the Creature</li>
	 * <li>Break a cast and send Server->Client ActionFailed packet and a System
	 * Message to the Creature</li>
	 * <li>Launch actions corresponding to the Event onAttacked (only for
	 * L2AttackableAI after the stunning periode)</li>
	 * </ul>
	 */
	@Override
	...
	/**
	 * Launch actions corresponding to the Event Paralyzed then onAttacked
	 * Event.
	 * <ul>
	 * <li>Stop the actor auto-attack client side by sending Server->Client
	 * packet AutoAttackStop (broadcast)</li>
	 ...
	 * <ul>
	 * <li>Stop an AI Follow Task</li>
	 * <li>Kill the actor client side by sending Server->Client packet
	 * AutoAttackStop, StopMove/StopRotation, Die (broadcast)</li>
	 * </ul>
	 */
	@Override
	 ...
	 * Interact area.
	 * <ul>
	 * <li>Get the distance between the current position of the Creature and the
	 * target (x,y)</li>
	 * <li>If the distance > offset+20, move the actor (by running) to Pawn
	 * server side AND client side by sending Server->Client packet MoveToPawn
	 * (broadcast)</li>
	 * <li>If the distance
	 * <= offset+20, Stop the actor movement server side AND client side by sending Server->Client
	 * packet StopMove/StopRotation (broadcast)</li>
	 * </ul>
	 *
	...
	public boolean canAOE(L2Skill sk) {
		if (sk.getSkillType() != ESkillType.NEGATE || sk.getSkillType() != ESkillType.CANCEL) {
			if (sk.getTargetType() == ESkillTargetType.TARGET_AURA || sk.getTargetType() == ESkillTargetType.TARGET_BEHIND_AURA || sk.getTargetType() == ESkillTargetType.TARGET_FRONT_AURA) {
