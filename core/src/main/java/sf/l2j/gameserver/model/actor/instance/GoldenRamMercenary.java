...
/**
 * This instance leads behaviors of Golden Ram mofos, where shown htm is
 * different according to your quest condition. Abercrombie shows you
 * multisells, Selina shows you Buffs list, when Pierce shows you "Quest"
 * link.<br>
 * <br>
		...
		if (actualCommand.contains("buff")) {
			if (qs != null && qs.getInt("cond") == 3) {
				...
				if (qs.getQuestItemsCount(GOLDEN_RAM) >= coins) {
					...
					setTarget(player);
					doCast(SkillTable.getInstance().getInfo(buffData[0], buffData[1]), false);
					val = 4;
				}
