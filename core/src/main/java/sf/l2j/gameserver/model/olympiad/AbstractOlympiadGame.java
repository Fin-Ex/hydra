...
import sf.l2j.gameserver.model.zone.type.L2OlympiadStadiumZone;
...
public abstract class AbstractOlympiadGame {
	...
	/**
	 * Return null if player passed all checks or broadcast the reason to
	 * opponent.
	 *
	...
	protected static final void removals(Player player, boolean removeParty) {
		try {
			...
			if (removeParty) {
				...
				if (party != null) {
					party.removePartyMember(player, EPartyMessageType.EXPELLED);
				}
			}
	...
	protected static final void cleanEffects(Player player) {
		try {
			...
			player.setCurrentHp(player.getMaxHp());
			player.setCurrentMp(player.getMaxMp());
			player.getStatus().startRegen(ERegenType.VALUES);
		} catch (Exception e) {
			_log.warn(e.getMessage(), e);
		}
	}
