...
import sf.l2j.gameserver.instancemanager.SevenSignsFestival.FestivalType;
		...
		if (command.startsWith("FestivalDesc")) {
			...
			switch (val) {
				...
				case 9: // Leave the Festival
					...
					/**
					 * If the player is the party leader, remove all
					 * participants from the festival (i.e. set the party to
					 * null, when updating the participant list) otherwise just
					 * remove this player from the "arena", and also remove them
					 * from the party.
					 */
					...
					if (isLeader) {
						...
						playerParty.removePartyMember(player, EPartyMessageType.EXPELLED);
					}
					break;
