package sf.l2j.gameserver.taskmanager;
 ...
 * shutdown/restart, saves all dropped items in to SQL. Loads them during server
 * start.
 */
public final class ItemsOnGroundTaskManager implements Runnable {

	private static final Logger LOGGER = LoggerFactory.getLogger(ItemsOnGroundTaskManager.class.getName());
	...
	public ItemsOnGroundTaskManager() {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); ResultSet rs = con.createStatement().executeQuery(LOAD_ITEMS)) {
			...
			LOGGER.warn("ItemsOnGroundTaskManager: Error while loading \"items_on_ground\" table: " + e.getMessage());
		}
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement st = con.prepareStatement(DELETE_ITEMS)) {
			...
			LOGGER.warn("ItemsOnGroundTaskManager: Can not empty \"items_on_ground\" table to save new items: " + e.getMessage());
		}
	}
	...
	public final void save() {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement st = con.prepareStatement(SAVE_ITEMS)) {
			...
			LOGGER.warn("ItemsOnGroundTaskManager: Could not save items on ground to \"items_on_ground\" table: " + e.getMessage());
		}
	}
