package sf.l2j.gameserver.geoengine;
...
import sf.l2j.gameserver.geoengine.geodata.BlockComplex;
import sf.l2j.gameserver.geoengine.geodata.BlockComplexDynamic;
import sf.l2j.gameserver.geoengine.geodata.BlockFlat;
import sf.l2j.gameserver.geoengine.geodata.BlockMultilayer;
import sf.l2j.gameserver.geoengine.geodata.BlockMultilayerDynamic;
import sf.l2j.gameserver.geoengine.geodata.BlockNull;
import sf.l2j.gameserver.geoengine.geodata.GeoFormat;
import sf.l2j.gameserver.geoengine.geodata.GeoLocation;
import sf.l2j.gameserver.geoengine.geodata.GeoStructure;
import sf.l2j.gameserver.geoengine.geodata.IBlockDynamic;
import sf.l2j.gameserver.geoengine.geodata.IGeoObject;
...
public class GeoEngine {
	...
	protected static final Logger _log = LoggerFactory.getLogger(GeoEngine.class.getName());
	...
	public GeoEngine() {
		...
		try {
			...
			_log.warn("GeoEngine: Could not load \"geo_bugs.txt\" file.");
		}
	...
	/**
	 * Loads geodata from a file. When file does not exist, is corrupted or not
	 * consistent, loads none geodata.
	 *
	...
	private final boolean loadGeoBlocks(int regionX, int regionY) {
		...
		try (RandomAccessFile raf = new RandomAccessFile(filepath, "r"); FileChannel fc = raf.getChannel()) {
			...
			if (buffer.remaining() > 0) {
				_log.warn("GeoEngine: Region file " + filename + " can be corrupted, remaining " + buffer.remaining() + " bytes to read.");
			}
			...
			_log.warn("GeoEngine: Error while loading " + filename + " region file.");
	...
	/**
	 * Loads null blocks. Used when no region file is detected or an error
	 ...
	 * @param geoY : Cell geodata Y coordinate.
	 * @param worldZ : Cell world Z coordinate.
	 * @return short : Cell NSWE flag byte coordinate, closest to given
	 * coordinates.
	 */
	...
	/**
	 * Returns the NSWE flag byte of cell, which is closes to given
	 * coordinates.<br>
	 * Geodata without {@link IGeoObject} are taken in consideration.
	 *
	 * @param geoX : Cell geodata X coordinate.
	...
	// GEODATA - DYNAMIC
	/**
	 * Returns calculated NSWE flag byte as a description of
	 * {@link IGeoObject}.<br>
	 * The {@link IGeoObject} is defined by boolean 2D array, saying if the
	 * object is present on given cell or not.
	 *
	 ...
	 * NOTE: When two doors close between each other and the LoS check of one
	 * doors is performed through another door, result will not be accurate (the
	 * other door are skipped).
	 *
	 * @param gox : origin X geodata coordinate
	 * @param goy : origin Y geodata coordinate
	 * @param goz : origin Z geodata coordinate
	 ...
	 * @param gty : target Y geodata coordinate
	 * @param gtz : target Z geodata coordinate
	 * @param theight : target height (if instance of {@link Creature} or
	 * {@link Door})
	 * @return {@code boolean} : True, when target can be seen.
	 */
	 ...
	 * @return {code boolean} : True if target coordinates are reachable from
	 * origin coordinates
	 */
	...
	/**
	 * Check movement from origin to target. Returns last available point in the
	 * checked path.
	 *
	 * @param ox : origin X coordinate
	 * @param oy : origin Y coordinate
	 ...
	 * @param ty : target Y coordinate
	 * @param tz : target Z coordinate
	 * @return {@link Location} : Last point where object can walk (just before
	 * wall)
	 */
	...
	/**
	 * With this method you can check if a position is visible or can be reached
	 * by beeline movement.<br>
	...
	/**
	 * Returns the list of location objects as a result of complete path
	 * calculation.
	 *
	 * @param ox : origin x
	 * @param oy : origin y
	...
	/**
	 * Return pathfinding statistics, useful for getting information about
	 * pathfinding status.
	 *
	...
	public final boolean addGeoBug(Location loc, String comment) {
		...
		try {
			...
			_log.warn("GeoEngine: Could not save new entry to \"geo_bugs.txt\" file.");
			return false;
		}
	}
