...
public final class DocumentItem extends DocumentBase {
				...
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
					if ("item".equalsIgnoreCase(d.getNodeName())) {
						try {
							...
							log.warn("Cannot create item " + _currentItem.id, e);
						}
					}
				}
			}
	...
	private void makeItem() throws InvocationTargetException {
		...
		try {
			Constructor<?> c = Class.forName("sf.l2j.gameserver.model.item.kind." + _currentItem.type).getConstructor(StatsSet.class);
