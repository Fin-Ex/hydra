...
import sf.l2j.gameserver.model.actor.instance.SepulcherMonster;
import sf.l2j.gameserver.model.actor.instance.SepulcherNpc;
...
public class FourSepulchersManager {
	...
	protected static final Logger _log = LoggerFactory.getLogger(FourSepulchersManager.class.getName());
		...
		for (Entry<Integer, Integer> keyNpc : _keyBoxNpc.entrySet()) {
			try {
				...
				if (template != null) {
					...
					_log.warn("FourSepulchersManager.InitKeyBoxSpawns: Data missing in NPC table for ID: " + keyNpc.getValue() + ".");
				}
			} catch (Exception e) {
		...
		for (Entry<Integer, Integer> victimNpc : _victim.entrySet()) {
			try {
				...
				if (template != null) {
					...
					_log.warn("FourSepulchersManager.InitExecutionerSpawns: Data missing in NPC table for ID: " + victimNpc.getValue() + ".");
				}
			} catch (Exception e) {
