...
public class RaidBossSpawnManager {
	...
	protected static final Logger _log = LoggerFactory.getLogger(RaidBossSpawnManager.class.getName());
	...
	private void init() {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			_log.warn("Error while initializing RaidBossSpawnManager: " + e.getMessage(), e);
		}
	}
	...
	public void addNewSpawn(L2Spawn spawnDat, long respawnTime, double currentHP, double currentMP, boolean storeInDb) {
		...
		if (storeInDb) {
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				...
				_log.warn("RaidBossSpawnManager: Could not store raidboss #" + bossId + " in the DB:" + e.getMessage(), e);
			}
		}
	}
	...
	public void deleteSpawn(L2Spawn spawnDat, boolean updateDb) {
		...
		if (updateDb) {
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				...
				_log.warn("RaidBossSpawnManager: Could not remove raidboss #" + bossId + " from DB: " + e.getMessage(), e);
			}
		}
	}
	...
	private void updateDb() {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			_log.warn("RaidBossSpawnManager: Couldnt update raidboss_spawnlist table " + e.getMessage(), e);
		}
	}
