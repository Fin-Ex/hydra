...
import sf.l2j.gameserver.model.L2Spawn;
...
import sf.l2j.gameserver.model.zone.type.L2PeaceZone;
...
public class SevenSignsFestival {
	...
	/**
	 * These length settings are important! :) All times are relative to the
	 * ELAPSED time (in ms) since a festival begins. Festival manager start is
	 * the time after the server starts to begin the first festival cycle. The
	 * cycle length should ideally be at least 2x longer than the festival
	 * length. This allows ample time for players to sign-up to participate in
	 * the festival. The intermission is the time between the festival
	 * participants being moved to the "arenas" and the spawning of the first
	 * set of mobs. The monster swarm time is the time before the monsters swarm
	 * to the center of the arena, after they are spawned. The chest spawn time
	 * is for when the bonus festival chests spawn, usually towards the end of
	 * the festival.
	 */
	...
	/**
	 * Primarily used to terminate the Festival Manager, when the Seven Signs
	 * period changes.
	 *
	...
	/**
	 * Used to start the Festival Manager, if the current period is not Seal
	 * Validation.
	 */
	...
	/**
	 * Restores saved festival data, basic settings from the properties file and
	 * past high score data from the database.
	 */
	...
	/**
	 * Stores current festival data, basic settings to the properties file and
	 * past high score data to the database.<BR>
	 * <BR>
	 * If updateSettings = true, then all Seven Signs data is updated in the
	 * database.
	 *
	...
	private static void addReputationPointsForPartyMemberClan(String playerName) {
		...
		if (player != null) {
			...
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				...
				_log.warn("could not get clan name of " + playerName + ": " + e);
			}
		}
	}

	/**
	 * Used to reset all festival data at the beginning of a new quest event
	 * period.
	 *
	...
	/**
	 * Returns the current festival ID and oracle ID that the specified player
	 * is in, but will return the default of {-1, -1} if the player is not found
	 * as a participant.
	 *
	 * @param player
	...
	public void updateParticipants(Player player, Party festivalParty) {
		...
		if (festivalId > -1) {
			...
			if (festivalParty != null && festivalParty.getMembersCount() < Config.ALT_FESTIVAL_MIN_PLAYER) {
				...
				festivalParty.removePartyMember(player, EPartyMessageType.EXPELLED);
			}
		}
	}
	...
	/**
	 * Returns a stats set containing the highest score <b>this cycle</b> for
	 * the the specified cabal and associated festival ID.
	 *
	 * @param oracle
	 * @param festivalId
	...
	/**
	 * Returns a stats set containing the highest ever recorded score data for
	 * the specified festival.
	 *
	 * @param festivalId
	...
	/**
	 * Set the final score details for the last participants of the specified
	 * festival data. Returns <b>true</b> if the score is higher than that
	 * previously recorded <b>this cycle</b>.
	 *
	 * @param player
	 * @param oracle
	 * @param festival
	...
	/**
	 * Calculate and return the proportion of the accumulated bonus for the
	 * festival where the player was in the winning party, if the winning
	 * party's cabal won the event. The accumulated bonus is then updated, with
	 * the player's share deducted.
	 *
	 * @param player
	...
	/**
	 * Basically a wrapper-call to signal to increase the challenge of the
	 * specified festival.
	 *
	 * @param oracle
	 * @param festivalId
	...
	/**
	 * Used to send a "shout" message to all players currently present in an
	 * Oracle. Primarily used for Festival Guide and Witch related speech.
	 *
	 * @param senderName
	 * @param message
	 */
	...
	/**
	 * The FestivalManager class is the main runner of all the festivals. It is
	 * used for easier integration and management of all running festivals.
	 *
	 * @author Tempy
	 */
	private class FestivalManager implements Runnable {
		...
		/**
		 * Returns the running instance of a festival for the given Oracle and
		 * festivalID. <BR>
		 * A <B>null</B> value is returned if there are no participants in that
		 * festival.
		 *
		 * @param oracle
		 * @param festivalId
	...
	/**
	 * Each running festival is represented by an L2DarknessFestival class. It
	 * contains all the spawn information and data for the running festival. All
	 * festivals are managed by the FestivalManager class, which must be
	 * initialized first.
	 *
	 * @author Tempy
	 */
	private class L2DarknessFestival {
		 ...
		 * 1 - Same as 0, but without archers/marksmen. (used for challenge
		 * increase) <BR>
