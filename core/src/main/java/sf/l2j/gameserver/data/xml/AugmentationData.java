...
import sf.l2j.gameserver.model.L2Augmentation;
import sf.l2j.gameserver.model.holder.IntIntHolder;
import sf.l2j.gameserver.network.clientpackets.AbstractRefinePacket;
 ...
 * <ul>
 * <li>{@link AugmentationStat} under 4 different tables of stats (pDef,
 * etc)</li>
 * <li>Augmentation skills based on colors (blue, purple and red)</li>
 * </ul>
 * It is also used to generate new {@link L2Augmentation}, based on stored
 * content.
 */
	...
	public List<AugStat> getAugStatsById(int augmentationId) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (stats[i] >= STAT_START && stats[i] <= STAT_END) {
				...
				switch (stats[i]) {
					case BASESTAT_STR:
						temp.add(new AugStat(Stats.STR, 1.0f));
						break;
					case BASESTAT_CON:
						temp.add(new AugStat(Stats.CON, 1.0f));
						break;
					case BASESTAT_INT:
						temp.add(new AugStat(Stats.INT, 1.0f));
						break;
					case BASESTAT_MEN:
						temp.add(new AugStat(Stats.MEN, 1.0f));
						break;
				}
			}
		}
