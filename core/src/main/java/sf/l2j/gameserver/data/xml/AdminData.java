 ...
 * <ul>
 * <li>{@link AccessLevel}s retain informations such as isGM() and multiple
 * allowed actions.</li>
 * <li>Admin command rights' authorized {@link AccessLevel}.</li>
 * <li>GM list holds GM {@link Player}s used by /gmlist. It also stores the
 * hidden state.</li>
 * </ul>
 */
public final class AdminData extends XMLDocument {
	...
	/**
	 * @return the master {@link AccessLevel}. It is always the AccessLevel with
	 * the highest level.
	 */
	 ...
	 * @return true if an AccessLevel can use a specific command (set as
	 * parameter).
	 */
	public boolean hasAccess(String command, AccessLevel accessToCheck) {
		...
		if (level == null) {
			LOG.error("No rights defined for admin command " + command + " !");
			return false;
		}
	 ...
	 * @return the List of GM {@link Player}s. This List can include or not
	 * hidden GMs.
	 */
	...
	/**
	 * @param includeHidden : If true, we add hidden GMs.
	 * @return the List of GM {@link Player}s names. This List can include or
	 * not hidden GMs.
	 */
	...
	/**
	 * Send the GM list of current online GM {@link Player}s to the Player set
	 * as parameter.
	 *
