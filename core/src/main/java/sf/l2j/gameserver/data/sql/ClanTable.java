...
public class ClanTable {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ClanTable.class.getName());
	...
	/**
	 * Instantly delete related clan. Run different clanId related queries,
	 * remove clan from _clans, inform clan members, delete clans from pending
	 * sieges.
	 *
	...
	/**
	 * Refresh clans ladder, picking up the 99 first best clans, and allocating
	 * their ranks accordingly.
	 *
	 * @param cleanupRank if true, cleanup ranks. Used for the task, useless for
	 * startup.
	 */
	public void refreshClansLadder(boolean cleanupRank) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			LOG.error("Error updating clans ladder.", e);
		}
	}
