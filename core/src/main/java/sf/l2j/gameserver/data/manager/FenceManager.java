package sf.l2j.gameserver.data.manager;
...
import sf.l2j.gameserver.model.actor.instance.Fence;
...
public class FenceManager {
	...
	private static final Logger LOG = LoggerFactory.getLogger(FenceManager.class.getName());

	/**
	 * Description of each fence dimension parameters.
	 * <ul>
	 * <li>_offset parameter says, what is the fence position compared to
	 * geodata grid.</li>
	 * <li>_geoDataSize parameter says, what is the raw fence size in geodata
	 * coordinates (cells).</li>
	 * </ul>
	 */
	 ...
	 * @param type : Type of the fence. 1..corner stones only, 2..fence + corner
	 * stones
	...
	public final Fence addFence(int x, int y, int z, int type, int sizeX, int sizeY, int height) {
		...
		if (fsx == null || fsy == null) {
			LOG.warn("Unknown dimensions for fence, x=" + sizeX + " y=" + sizeY);
			return null;
		}
	 ...
	 * @return {@link FenceSize} : Size of {@link Fence} in particular
	 * dimension.
	 */
