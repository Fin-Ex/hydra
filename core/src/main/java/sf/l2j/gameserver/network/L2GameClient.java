...
import sf.l2j.gameserver.model.CharSelectInfoPackage;
...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable {
	 ...
	 * @return a byte:
	 * <li>-1: Error: No char was found for such charslot, caught exception,
	 * etc...
	...
	public static void deleteCharByObjId(int objid) {
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
			...
			statement = con.prepareStatement("DELETE FROM character_craft WHERE objectId=?");
			statement.setInt(1, objid);
			statement.execute();
			statement.close();

			statement = con.prepareStatement("DELETE FROM character_talents WHERE objectId=?");
			statement.setInt(1, objid);
			statement.execute();
			statement.close();

			statement = con.prepareStatement("DELETE FROM character_lineage WHERE objectId=?");
			statement.setInt(1, objid);
			statement.execute();
			statement.close();
	...
	public Player loadCharFromDisk(int slot) {
		...
		if (player != null) {
			...
			_log.error("Attempt of double login: " + player.getName() + "(" + objectId + ") " + getAccountName());
		...
		if (player != null) {
			...
			_log.error("L2GameClient: could not restore in slot: " + slot);
		}
...
//	}
			...
			try {
				...
				if (getActiveChar() != null) // this should only happen on connection loss
				{
					if (getActiveChar().isLocked()) {
						_log.warn(getActiveChar().getName() + " is still performing subclass actions during disconnect.");
					}
			...
			try {
				if (getActiveChar() != null && getActiveChar().isOnline()) {
					...
					if (getActiveChar().getActiveSummon() != null) {
						getActiveChar().getActiveSummon().store();
					}
				}
			} catch (Exception e) {
	...
	/**
	 * @return false if client can receive packets. True if detached, or flood
	 * detected, or queue overflow detected and queue still not empty.
	 */
