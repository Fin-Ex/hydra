...
import sf.l2j.gameserver.network.serverpackets.GMViewItemList;
...
public class AdminEditChar implements IAdminCommandHandler {
	...
	public boolean useAdminCommand(String command, Player activeChar) {
		if (command.startsWith("admin_changelvl")) {
			...
			if (target instanceof Summon) {
				...
				Summon pet = ((Player) target).getActiveSummon();
				if (pet != null) {
			...
			WorldObject target = activeChar.getTarget();
			if (target instanceof Summon) {
				((Summon) target).unSummon(((Summon) target).getPlayer());
			} else {
				activeChar.sendPacket(SystemMessageId.INCORRECT_TARGET);
			}
	...
	/**
	 * Find the character based on his name, and send back the result to
	 * activeChar.
	 *
	...
	private static void gatherSummonInfo(Summon target, Player activeChar) {
		...
		final String owner = target.getPlayer().getName();
		...
		if (target instanceof Pet) {
			...
			html.replace("%inv%", " <a action=\"bypass admin_show_pet_inv " + target.getPlayer().getObjectId() + "\">view</a>");
