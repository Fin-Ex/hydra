package sf.l2j.gameserver.scripting.scripts.ai;
...
public class L2AttackableAIScript extends Quest {
	...
	protected void registerNpcs() {
		...
		for (NpcTemplate template : NpcTable.getInstance().getAllNpcs()) {
			try {
				if (template.getType().equalsIgnoreCase("Pet") || template.getType().equalsIgnoreCase("Servitor")) {
					continue;
				}

				final Class clazz = Class.forName("sf.l2j.gameserver.model.actor.instance." + template.getType());
				if (Attackable.class.isAssignableFrom(clazz)) {
				...
				log.warn("Class not found: {}", template.getType(), ex);
			}
		}
	}
	...
	public String onSkillSee(Npc npc, Player caster, L2Skill skill, WorldObject[] targets, boolean isPet) {
		...
		if (caster.getActiveSummon() != null) {
			if (targets.length == 1 && ArraysUtil.contains(targets, caster.getActiveSummon())) {
				skillAggroPoints = 0;
			}
		}
			...
			if (attackable.hasAI() && (attackable.getAI().getIntention() == CtrlIntention.ATTACK)) {
				...
				for (WorldObject skillTarget : targets) {
					if (npcTarget == skillTarget || npc == skillTarget) {
						Creature originalCaster = isPet ? caster.getActiveSummon() : caster;
	...
	public String onFactionCall(Npc npc, Npc caller, Player attacker, boolean isPet) {
		...
		final Creature originalAttackTarget = (isPet ? attacker.getActiveSummon() : attacker);
	...
	public String onAggro(Npc npc, Player player, boolean isPet) {
		...
		((Attackable) npc).addDamageHate(isPet ? player.getActiveSummon() : player, 0, 1);
		return null;
	}
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isPet, L2Skill skill) {
		if (attacker != null && npc instanceof Attackable) {
			Attackable attackable = (Attackable) npc;
			Creature originalAttacker = isPet ? attacker.getActiveSummon() : attacker;
	...
	/**
	 * Under that barbarian name, return the number of players in front, back
	 * and sides of the npc.<br>
	 * Dead players aren't counted, invisible ones is the boolean parameter.
	 *
	 * @param range : the radius.
	 * @param npc : the object to make the test on.
