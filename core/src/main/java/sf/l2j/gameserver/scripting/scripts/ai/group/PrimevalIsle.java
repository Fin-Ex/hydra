 ...
 * <ul>
 * <li>Sprigant : casts a spell if you enter in aggro range, finish task if die
 * or none around.</li>
...
public class PrimevalIsle extends L2AttackableAIScript {
	...
	public String onAdvEvent(String event, Npc npc, Player player) {
		...
		if (event.equalsIgnoreCase("skill")) {
			...
			npc.setTarget(npc);
			npc.doCast((npc.getNpcId() == 18345) ? ANESTHESIA : POISON, false);
		}
		return null;
	}
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isPet, L2Skill skill) {
		...
		final Playable originalAttacker = (isPet ? attacker.getActiveSummon() : attacker);
