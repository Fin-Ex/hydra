...
public abstract class ScheduledQuest extends Quest {
	...
	/**
	 * Set up schedule system for the script. Returns true, when successfully
	 * done.
	 *
	...
	public final boolean setSchedule(String type, String start, String end) {
		try {
			...
			_log.warn(getName() + ": Error while loading schedule data: " + e);
			_type = null;
			_start = null;
			_end = null;
			_started = false;
	...
	public final void notifyAndSchedule() {
		...
		if (_end == null) {
			...
			try {
				...
				_log.warn(getName() + ": Error while starting the script: " + e.getMessage());
			}
		...
		if (_started) {
			...
			try {
				...
				_log.warn(getName() + ": Error while ending the script: " + e.getMessage());
			}
	...
	/**
	 * Initializes a script and returns information about script to be scheduled
	 * or not. Set internal values, parameters, etc...
	 *
