package sf.l2j.gameserver.scripting.scripts.ai.individual;
...
/**
 * Benom is a specific Raid Boss, appearing in Rune Castle. He is aggressive
 * towards anyone.<br>
 * <br>
 * The castle owning clan can defeat Benom. It can teleport to Benom's den using
 * a specific gatekeeper, 24 hours before siege start. If the clan doesn't kill
 * Benom before the siege start, Benom will appear during the siege if at least
 * 2 life controls crystals have been broken.
 */
public class Benom extends L2AttackableAIScript {
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isPet, L2Skill skill) {
		if (Rnd.get(100) <= 25) {
			...
			npc.doCast(SkillTable.getInstance().getInfo(4995, 1), false);
		} else if (!npc.isCastingNow()) {
			if ((npc.getCurrentHp() < (npc.getMaxHp() / 3)) && Rnd.get(500) < 1) {
				npc.setTarget(attacker);
				npc.doCast(SkillTable.getInstance().getInfo(4996, 1), false);
			} else if (!npc.isInsideRadius(attacker, 300, true, false) && Rnd.get(100) < 1) {
				npc.setTarget(attacker);
				npc.doCast(SkillTable.getInstance().getInfo(4993, 1), false);
			} else if (Rnd.get(100) < 1) {
				npc.setTarget(attacker);
				npc.doCast(SkillTable.getInstance().getInfo(4994, 1), false);
			}
		}
