package sf.l2j.gameserver.scripting.scripts.custom;
...
import sf.l2j.gameserver.network.serverpackets.WarehouseWithdrawList;
 ...
 * <li>Petrification effect in case an allied player helps a neutral or
 * enemy.</li>
 * </ul>
 */
public class KetraOrcSupport extends Quest {
	...
	/**
	 * Names of missions which will be automatically dropped if the alliance is
	 * broken.
	 */
	...
	public String onAdvEvent(String event, Npc npc, Player player) {
		...
		if (StringUtil.isDigit(event)) {
			...
			if (st.getQuestItemsCount(HORN) >= buffInfo[1]) {
				...
				npc.doCast(SkillTable.getInstance().getInfo(buffInfo[0], 1), false);
					...
					for (Creature target : (Creature[]) targets) {
						...
						if (!(player.isAlliedWithKetra())) {
							...
							if (((Attackable) npc).getAggroList().containsKey(player)) {
								...
								npc.setTarget((isPet && player.getActiveSummon() != null) ? caster.getActiveSummon() : caster);
								npc.doCast(FrequentSkill.VARKA_KETRA_PETRIFICATION.getSkill(), false);
	 ...
	 * If any Varka quest is in progress, it stops the quest (and drop all
	 * related qItems) :
	 *
