package sf.l2j.gameserver.scripting;
...
import sf.l2j.gameserver.network.serverpackets.ExShowQuestMark;
...
import sf.l2j.gameserver.network.serverpackets.TutorialCloseHtml;
import sf.l2j.gameserver.network.serverpackets.TutorialEnableClientEvent;
import sf.l2j.gameserver.network.serverpackets.TutorialShowHtml;
import sf.l2j.gameserver.network.serverpackets.TutorialShowQuestionMark;
...
public final class QuestState {
	 ...
	 * Completion, State)</LI>
	 * <LI>Add the QuestState in the player's list of quests by using
	 * setQuestState()</LI>
	 ...
	 * <LI>If the key represented by "var" exists in FastMap "vars", the couple
	 * (var,val) is updated in the database. The key is known as existing if the
	 * preceding value of the key (given as result of function put()) is not
	 * null.<BR>
	 * If the key doesn't exist, the couple is added/created in the
	 * database</LI>
	 *
	...
	public void set(String var, String value) {
		...
		if ("cond".equals(var)) {
			try {
				...
				_log.warn(_player.getName() + ", " + _quest.getName() + " cond [" + value + "] is not an integer. Value stored, but no packet was sent: " + e.getMessage(), e);
			}
		}
	}
	 ...
	 * <LI>If skipped, prepare the variable completedStateFlags appropriately to
	 * be ready for sending to clients</LI>
	 ...
	 * @param old : int indicating the previously noted step For more info on
	 * the variable communicating the progress steps to the client, please see
	 */
	...
	public int getInt(String var) {
		...
		try {
			...
			_log.debug(_player.getName() + ": variable " + var + " isn't an integer: " + value + " ! " + e.getMessage(), e);
		}
	 ...
	 * @return {@code true} if the item exists in player's inventory,
	 ...
	 * @return {@code true} if all items exist in player's inventory,
	 * {@code false} otherwise
	 */
	 ...
	 *
	 * @param itemIds a list of item IDs to check for
	 * @return {@code true} if at least one item exists in player's inventory,
	 * {@code false} otherwise
	 */
	...
	/**
	 * Drop items to the player's inventory. Rate is 100%, amount is affected by
	 * Config.RATE_QUEST_DROP.
	 *
	 * @param itemId : Identifier of the item to be dropped.
	 * @param count : Quantity of items to be dropped.
	 * @param neededCount : Quantity of items needed to complete the task. If
	 * set to 0, unlimited amount is collected.
	...
	/**
	 * Drop items to the player's inventory. Rate and amount is affected by
	 * DIVMOD of Config.RATE_QUEST_DROP.
	 *
	 * @param itemId : Identifier of the item to be dropped.
	 * @param count : Quantity of items to be dropped.
	 * @param neededCount : Quantity of items needed to complete the task. If
	 * set to 0, unlimited amount is collected.
	 * @param dropChance : Item drop rate (100% chance is defined by the
	 * L2DropData.MAX_CHANCE = 1.000.000).
	 * @return boolean : Indicating whether item quantity has been reached.
	 */
	 ...
	 * @param dropChance : Item drop rate (100% chance is defined by the
	 * L2DropData.MAX_CHANCE = 1.000.000).
	 * @param type : Item drop behavior: DROP_DIVMOD (rate and),
	 * DROP_FIXED_RATE, DROP_FIXED_COUNT or DROP_FIXED_BOTH
	 * @return boolean : Indicating whether item quantity has been reached.
	 */
	...
	/**
	 * Drop multiple items to the player's inventory. Rate and amount is
	 * affected by DIVMOD of Config.RATE_QUEST_DROP.
	 *
	 * @param rewardsInfos : Infos regarding drops (itemId, count, neededCount,
	 * dropChance).
	 * @return boolean : Indicating whether item quantity has been reached.
	 */
	...
	/**
	 * Reward player with items. The amount is affected by
	 * Config.RATE_QUEST_REWARD or Config.RATE_QUEST_REWARD_ADENA.
	 *
	 * @param itemId : Identifier of the item.
	...
	/**
	 * Reward player with EXP and SP. The amount is affected by
	 * Config.RATE_QUEST_REWARD_XP and Config.RATE_QUEST_REWARD_SP
	 *
	...
	/**
	 * Reward player by items from quest_data.json file. That reward depends
	 * from Config.RATE_QUEST_REWARD or Config.RATE_QUEST_REWARD_ADENA
	 */
	public void giveDataReward() {
		if (_quest.getData() == null || _quest.getData().getRewards() == null) {
			return;
		}

		_quest.getData().getRewards().forEach(rewardData -> {
			int count = rewardData.getCount();
			if (rewardData.isMultipliable()) {
				if (rewardData.getId() == 57) {
					count *= Config.RATE_QUEST_REWARD_ADENA;
				} else {
					count *= Config.RATE_QUEST_REWARD;
				}
			}
			giveItems(rewardData.getId(), count);
		});
	}
	 ...
	 * from Config.RATE_QUEST_REWARD or Config.RATE_QUEST_REWARD_ADENA
	 */
	public void giveDataExpSp() {
		if (_quest.getData() == null) {
			return;
		}

		rewardExpAndSp((int) (_quest.getData().getExp() * Config.RATE_QUEST_REWARD_XP), (int) (_quest.getData().getSp() * Config.RATE_QUEST_REWARD_SP));
	}
