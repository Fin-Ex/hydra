...
public class Baium extends L2AttackableAIScript {
		...
		if (npc != null && npc.getNpcId() == LIVE_BAIUM) {
			...
			// also check if the players are cheating, having pulled Baium outside his zone...
				...
				if (_lastAttackTime + 1800000 < System.currentTimeMillis()) {
					...
					npc.setTarget(npc);
					npc.doCast(SkillTable.getInstance().getInfo(4135, 1), false);
	...
	/**
	 * This method allows to select a random target, and is used both for Baium
	 * and angels.
	 *
	 * @param npc to check.
	 ...
	 * If Baium feels surrounded, he will use AoE skills. Same behavior if he is
	 * near 2+ angels.<br>
	 *
	 * @param npc baium
