...
public final class SelectorThreadImpl<T extends MMOClient> extends Thread implements SelectorThread {
	...
	public SelectorThreadImpl(final SelectorConfig sc, final IMMOExecutor<T> executor, final IPacketHandler<T> packetHandler, final IClientFactory<T> clientFactory, final AcceptFilter acceptFilter) throws IOException {
		...
		if (!con.isClosed()) {
			...
			if (result > 0) {
				...
				if (buf.remaining() > 0) {
					...
					if (buf == READ_BUFFER) // move the pending byte to the connections READ_BUFFER
					{
						allocateReadBuffer(con);
					} else // move the first byte to the beginning :)
					{
						buf.compact();
					}
				}
