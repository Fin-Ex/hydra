...
public abstract class Item {
	...
	private final EGradeType _crystalType;
	...
	protected Item(StatsSet set) {
		...
		_crystalType = set.getEnum("crystal_type", EGradeType.class, EGradeType.NG);
		...
		final String skills = set.getString("item_skill", null);
		if (skills != null) {
			try {
				final String[] splitter = skills.split(";");
	...
	public final boolean isCrystallizable() {
		return _crystalType != EGradeType.NG && _crystalCount > 0;
	}

	/**
	 * @return CrystalType the type of crystal if item is crystallizable
	 */
	public final EGradeType getCrystalType() {
		return _crystalType;
	}
	...
	/**
	 * @param enchantLevel
	 * @return int the quantity of crystals for crystallization on specific
	 * enchant level
	 */
	...
	public boolean checkCondition(Creature activeChar, WorldObject target, boolean sendMessage) {
		...
		if ((isOlyRestrictedItem() || isHeroItem()) && ((activeChar instanceof Player) && activeChar.getPlayer().isInOlympiadMode())) {
			if (isEquipable()) {
				activeChar.getPlayer().sendPacket(SystemMessageId.THIS_ITEM_CANT_BE_EQUIPPED_FOR_THE_OLYMPIAD_EVENT);
			} else {
				activeChar.getPlayer().sendPacket(SystemMessageId.THIS_ITEM_IS_NOT_AVAILABLE_FOR_THE_OLYMPIAD_EVENT);
			}
		...
		for (Condition preCondition : _preConditions) {
			...
			if (!preCondition.test(env)) {
				if (activeChar instanceof Summon) {
					activeChar.getPlayer().sendPacket(SystemMessageId.PET_CANNOT_USE_ITEM);
					return false;
				}
...
//			statemask |= states[i].getMask();
//		}
//		return (statemask & stateType.getMask()) != 0;
//	}
}
