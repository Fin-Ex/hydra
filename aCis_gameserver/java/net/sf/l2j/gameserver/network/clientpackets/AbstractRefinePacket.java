...
public abstract class AbstractRefinePacket extends L2GameClientPacket {
	...
	protected static final boolean isValid(Player player, ItemInstance item, ItemInstance refinerItem, ItemInstance gemStones) {
		...
		final EGradeType grade = item.getItem().getCrystalType();
	...
	protected static final boolean isValid(Player player, ItemInstance item) {
		...
		if (item.getItem().getCrystalType().isLesser(EGradeType.C)) {
			return false;
		}
	 ...
	 */
	protected static final int getGemStoneId(EGradeType itemGrade) {
		switch (itemGrade) {
			case C:
			case B:
				return GEMSTONE_D;
	 ...
	 */
	protected static final int getGemStoneCount(EGradeType itemGrade) {
		switch (itemGrade) {
			case C:
				return 20;
