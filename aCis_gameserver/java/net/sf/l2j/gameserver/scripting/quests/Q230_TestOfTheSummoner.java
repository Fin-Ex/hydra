...
public class Q230_TestOfTheSummoner extends Quest {
	...
	public String onAdvEvent(String event, Npc npc, Player player) {
		...
		} else if (event.equals("30063-04.htm")) // Lara later to give a list out
		{
			...
			npc.doCast(SkillTable.getInstance().getInfo(4126, 1), false);
		} // CAMONIELL
	...
	public String onTalk(Npc npc, Player player) {
		...
		switch (st.getState()) {
			case STATE_CREATED:
				if (player.getClassId() != ClassId.HumanWizard && player.getClassId() != ClassId.ElvenWizard && player.getClassId() != ClassId.DarkWizard) // wizard, elven wizard, dark wizard
				{
		...
		switch (npcId) {
			case PAKO_THE_CAT:
				if (st.getInt("Almors") == 2 && isPet && npc.getCurrentHp() == npc.getMaxHp()) {
					...
					_duelsInProgress.put(npcId, new ProgressDuelMob(attacker, attacker.getActiveSummon()));
					...
					if (!isPet || attacker.getActiveSummon() != duel.getPet()) // if a foul occured find the player who had the duel in progress and give a foul crystal
					{
			...
			case UNICORN_RACER:
				if (st.getInt("Camoniell") == 2 && isPet && npc.getCurrentHp() == npc.getMaxHp()) {
					...
					ProgressDuelMob duel = _duelsInProgress.get(npcId);
					if (!isPet || attacker.getActiveSummon() != duel.getPet()) {
						Player foulPlayer = duel.getAttacker();
						if (foulPlayer != null) {
							st = foulPlayer.getQuestState(qn);
							if (st != null) {
