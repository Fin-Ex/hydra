			...
			if (!chest.isInteracted()) {
				...
				if (Rnd.get(100) < 40) {
					switch (skill.getId()) {
						...
						default:
							chest.doCast(SkillTable.getInstance().getInfo(4143, Math.min(10, Math.round(npc.getLevel() / 10))), false);
							break;
					}
				} // Mimic behavior : attack the caster.
				else {
					attack(chest, ((isPet) ? caster.getActiveSummon() : caster));
				}
			}
		}
		return super.onSkillSee(npc, caster, skill, targets, isPet);
		...
		if (npc instanceof Chest) {
			...
			if (!chest.isInteracted()) {
				...
				if (Rnd.get(100) < 40) {
					...
					attack(chest, ((isPet) ? attacker.getActiveSummon() : attacker), ((damage * 100) / (chest.getLevel() + 7)));
				}
			}
		}
